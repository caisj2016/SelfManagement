...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(WebSecurity web) throws Exception {
		...
		web.ignoring().antMatchers("/img/**","/static/**", "/css/**", "/plugins/**","/webjars/**","/js/**","/bootstrap/**");
	}

	@Override
	protected void configure(HttpSecurity http) throws Exception {
		// 認可の設定
		http.authorizeRequests().antMatchers("/", "/login","/index", "/join", "/loginIndex","/joinIndex").permitAll() // indexは全ユーザーアクセス許可
				.anyRequest().authenticated(); // それ以外は全て認証無しの場合アクセス不許可

		// ログイン設定
		http.formLogin().loginProcessingUrl("/login") // 認証処理のパス
				.loginPage("/loginIndex") // ログインフォームのパス
                .failureUrl("/loginIndex?error")
				.defaultSuccessUrl("/dashboard", true) // 認証成功時の遷移先
